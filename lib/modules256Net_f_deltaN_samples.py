...
class RadioNet(nn.Module):
    ...
    def __init__( self,inputs=6):
        ...
        self.encode1_conv = convrelu(inputs, 32)
    ...
    def forward(self, input, z=None):
        ...
        input0 = torch.zeros(input.shape[0], 6, input.shape[2], input.shape[3], device=input.device)
        input0[:, 0:3, :, :] = input[:, 0:3, :, :]
        input0[:, 3, :, :] = (model + material_contribution) / 255.0
        input0[:, 4, :, :] = input[:, 12, :, :]
        input0[:, 5, :, :] = input[:, 4, :, :]
